...
def menu_operation(strval):
	print"menu operation received val: "+str(strval)
	if ( strval == "Update"):
		with canvas(device) as draw:
			draw.rectangle(device.bounding_box, outline="white", fill="black")
...
def rotary_callback(channel):
	...
	try:
		...
		if dt_new_state==dtLastState and new_clk_state==clkLastState:
			print"bounce"
			return
		dtLastState=dt_new_state
		clkLastState=new_clk_state
							# for next bouncing check


		if(dtLastState and clkLastState):
			#LockRotary.acquire()
			if(channel==clk):
				counter += 1
			else:
			...
			with canvas(device) as draw:
				menu(device, draw, names,counter%6)
			#LockRotary.release()
...
clkLastState = 0
dtLastState=0
GPIO.add_event_detect(dt, GPIO.RISING , callback=rotary_callback)
