...
names = ['Disk', 'Memory', 'Network', 'CPUUsage', 'IPAddress', 'Update']
...
def menu_operation(strval):
    if ( strval == "Update"):
        ...
        with canvas(device) as draw:
            draw.rectangle(device.bounding_box, outline="white", fill="black")
	    draw.text((10, 20), "Updating..... ", fill="white")
	    draw.text((10, 30), "Please Wait..!", fill="white")
        rc = subprocess.call("inst.sh")
...
def rotary_callback(channel):
    ...
    global counter,num
    num=num+1
    print("called function"+str(num))
    try:
		new_clk_state=GPIO.input(clk)
		dt_new_state=GPIO.input(dt)
		if(clkLastState!= new_clk_state):
                    # or dtLastState != dt_new_state ):
		...
		if(dtLastState != dt_new_state):
			if(dtLastState==0 and dt_new_state==1 and clkLastState==0 and new_clk_state==0):
				counter -= 1
				print "downcount"
		...
		#clkLastState=new_clk_state
global num		#dtLastState=dt_new_state
num=0
counter = 0
insubmenu = 0
clkLastState = GPIO.input(clk)
dtLastState=GPIO.input(dt)
GPIO.add_event_detect(dt, GPIO.FALLING , callback=rotary_callback, bouncetime=200)
